# 面试知识库索引

> 计算机专业面试知识库，按分类整理所有知识点。

---

## 计算机网络

- [OSI 网络七层模型](计算机网络/OSI七层模型.md)
- [第一层：物理层](计算机网络/物理层.md)
- [第二层：数据链路层](计算机网络/数据链路层.md)
- [第三层：网络层](计算机网络/网络层.md)
- [第四层：传输层](计算机网络/传输层.md)
- [第五层：会话层](计算机网络/会话层.md)
- [第六层：表示层](计算机网络/表示层.md)
- [第七层：应用层](计算机网络/应用层.md)
- [对称加密、非对称加密与 HTTPS](计算机网络/加密体系与HTTPS.md)

## 操作系统

- [进程、线程、协程的区别与联系](操作系统/进程-线程-协程.md)
- [线程创建方式、生命周期与状态](操作系统/线程创建-生命周期-状态.md)
- [Linux 常用命令面试速查](操作系统/Linux-常用命令.md)
- [Linux 文本处理实战：取列、排序、去重](操作系统/Linux-日志分析实战.md)

## 数据结构与算法

## 数据库

- [MySQL 的 MVCC 机制](数据库/MySQL-MVCC.md)
- [Redis I/O 多路复用与网络 I/O 模型](数据库/Redis-IO多路复用.md)
- [Redis 缓存穿透、击穿、雪崩与预热](数据库/Redis-缓存穿透-击穿-雪崩-预热.md)

## 大模型 / AI

- [OpenClaw 与 Claude Code 的相同点与不同点](大模型-AI/OpenClaw-vs-Claude-Code.md)
- [AI Agent 的调优评估与评测集构建](大模型-AI/Agent-调优评估与评测集构建.md)
- [Agent 如何做意图识别](大模型-AI/Agent-意图识别.md)
- [Prompt Engineering 核心技巧](大模型-AI/Prompt-Engineering-核心技巧.md)
- [ReAct / Plan-and-Execute / Reflection 三种范式](大模型-AI/Agent-ReAct-PlanAndExecute-Reflection.md)
- [Agent 多轮对话自我调整机制](大模型-AI/Agent-多轮对话自我调整.md)
- [Agent 上下文过长怎么处理](大模型-AI/Agent-上下文过长.md)

## 系统设计

- [Kafka 消息队列核心原理](系统设计/Kafka-消息队列.md)

## 编程语言

- [JVM — Java 虚拟机](编程语言/JVM-%20Java虚拟机.md)
- [Java GC 垃圾回收详解](编程语言/Java-GC垃圾回收.md)

## 场景问题

> 场景类面试题，串联多个分类的知识点，给出完整答题线。

- [设计一个监控日志分析 Agent](场景问题/监控日志分析Agent设计.md)
- [10 张票 10 万人抢 — Redis + MySQL 秒杀方案](场景问题/Redis秒杀方案设计.md)

---

- [回答模板](TEMPLATE.md) — 可随时修改，影响后续所有回答的格式
- [项目说明](README.md)
